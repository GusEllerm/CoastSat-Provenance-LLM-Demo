...
async fn render_text_through_jinja(text: &str, executor: &mut Executor) -> Result<String> {
    ...
    if let Some(error_msg) = messages.iter().find(|msg| {
        matches!(
            msg.level,
            schema::MessageLevel::Error | schema::MessageLevel::Exception
        )
    }) {
    ...
    if let Some(Node::String(rendered)) = outputs.first() {
        ...
        tracing::warn!(
            "Jinja kernel returned unexpected output format: {:?}",
            outputs
        );
        Ok(text.to_string())
    }
